...
def predict(base_dir: Path, batch_size: int):
    ...
    preds = model.predict(batch, batch_size=batch_size, verbose=1)
...
def viz_gradcam(model, image, tf, save_path, interpolant=0.5):
    ...
    activation_map = cv2.resize(activation_map.numpy() if hasattr(activation_map, "numpy") else activation_map,
